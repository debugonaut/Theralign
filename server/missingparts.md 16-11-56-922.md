...
The minimum viable mobile treatment: single column layout, full-width inputs, full-width buttons with `56px` height for touch targets, the form centered with `24px` horizontal padding on each side. The card wrapper for the form gets `border-radius: 12px` on mobile — slightly more rounded than the `8px` desktop treatment because mobile interfaces at this size read card edges more sharply. The Theralign logotype at the top of the form, `32px`, centered. Nothing else. No hero section, no marketing copy, no left-column illustration. The mobile login page is the form and nothing but the form.
