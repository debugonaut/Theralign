...
const doctorSeeds = [
  // ─── PUNE (6 Doctors) ───────────────────────────────────────────────────────
  {
    ...
    profile: {
      ...
      degreeDocument: 'https://res.cloudinary.com/demo/image/upload/v1/sample.jpg',
      licenseDocument: 'https://res.cloudinary.com/demo/image/upload/v1/sample.jpg',
...
export const seedDoctors = async () => {
  try {
    ...
    if (existingCount >= 36) {
      ...
      // Update any doctors with old sample PDF URLs to use the live Cloudinary sample.pdf
      const oldDegreeUrl = 'https://res.cloudinary.com/demo/image/upload/v1/doctor_docs/degree_sample.pdf';
      const oldLicenseUrl = 'https://res.cloudinary.com/demo/image/upload/v1/doctor_docs/license_sample.pdf';
      const newPdfUrl = 'https://res.cloudinary.com/demo/image/upload/sample.pdf';
      
      const updateResult = await DoctorProfile.updateMany(
        { $or: [ { degreeDocument: oldDegreeUrl }, { licenseDocument: oldLicenseUrl } ] },
        { $set: { degreeDocument: newPdfUrl, licenseDocument: newPdfUrl } }
      );
