        ...
        <div className="flex border-2 border-swiss-black">
          {['all', 'patient', 'doctor'].map((role) => (
            <button
              ...
              className={`px-4 py-2 text-[10px] font-black uppercase tracking-widest border-r-2 last:border-r-0 border-swiss-black cursor-pointer ${
                roleFilter === role
      ...
      <div className="bg-swiss-white border-2 border-swiss-black rounded-none shadow-none text-left">
        {loading ? (
          <div className="p-12 text-center text-swiss-gray-400 text-xs font-bold uppercase tracking-wider">
            <span className="inline-block animate-spin mr-2">⏳</span> RETRIEVING USER ACCOUNTS...
          </div>
        ) : users.length === 0 ? (
              ...
              {users.map((u) => {
                ...
                return (
                  <Table.Row key={u._id}>
                    ...
                    {/* Email */}
                    <Table.Cell className="font-mono text-xs text-swiss-gray-500">
                      {u.email}
                    </Table.Cell>
                    ...
                    <Table.Cell>
                      {isUserActive ? (
                        <span 
                          ...
                          className="border-2 bg-swiss-white font-bold uppercase text-[11px] tracking-widest px-2 py-0.5"
                        >
                          ACTIVE
                        </span>
                      ) : (
                    ...
                    <Table.Cell actions={true}>
                      {isUserAdmin ? (
                        <span className="text-[10px] font-bold text-swiss-gray-400 uppercase tracking-widest italic pr-4">
                          PROTECTED
                        </span>
                      ) : isUserActive ? (
                        <ActionLink
                          onClick={() => handleToggleStatus(u._id)}
                          className="text-swiss-gray-400 hover:text-swiss-black hover:underline"
                        >
                          DEACTIVATE
                        </ActionLink>
                      ) : (
                        <ActionLink
                          onClick={() => handleToggleStatus(u._id)}
                          className="text-swiss-black hover:text-swiss-gray-650 hover:underline"
                        >
                          REACTIVATE
                        </ActionLink>
                      )}
